...
import {
  allCeramicCoatingLocations,
  getCeramicCoatingLocationBySlug,
} from '@/lib/data/ceramic-coating-locations';
import CeramicCoatingLocationPage from './CeramicCoatingLocationPage';

export const dynamicParams = false;

export async function generateStaticParams() {
  return [
    ...allLocations.map((loc) => ({ locationSlug: loc.slug })),
    ...allCeramicCoatingLocations.map((loc) => ({ locationSlug: loc.slug })),
  ];
}
...
export async function generateMetadata({
  ...
  const ccLocation = getCeramicCoatingLocationBySlug(locationSlug);
  if (ccLocation) {
    return {
      title: ccLocation.metaTitle,
      description: ccLocation.metaDescription,
      keywords: [
        `ceramic coating ${ccLocation.name}`,
        `ceramic coating ${ccLocation.postcode}`,
        `car paint protection ${ccLocation.name}`,
        `Gtechniq ${ccLocation.name}`,
        `CarPro Cquartz ${ccLocation.name}`,
        'ceramic coating Warrington',
        'crystal serum Warrington',
        'paint protection Warrington',
      ],
      alternates: { canonical: `${SITE_URL}/${ccLocation.slug}/` },
      openGraph: {
        title: ccLocation.metaTitle,
        description: ccLocation.metaDescription,
        url: `${SITE_URL}/${ccLocation.slug}/`,
        siteName: 'Car Ceramic Coating and detailing warrington0',
        locale: 'en_GB',
        type: 'website',
        images: [{ url: '/og-image.jpg', width: 1200, height: 630, alt: ccLocation.metaTitle }],
      },
      twitter: {
  ...
  // Car detailing location pages (existing)
...
const coreServices = [
  ...
  { title: 'Machine Polishing', slug: 'paint-correction', icon: '🔧', desc: 'Remove swirl marks, scratches, and oxidation with machine polishing.' },
...
const benefits = [
  { icon: '🏭', title: 'Unit Based', desc: 'Bring your vehicle to our unit at Fairclough Mill, Atherton\'s Quay, Warrington WA5 — professional results every time.' },
...
export default async function LocationPage({
  ...
  // Ceramic coating location — delegate to dedicated component
  const ccLocation = getCeramicCoatingLocationBySlug(locationSlug);
  if (ccLocation) {
    return <CeramicCoatingLocationPage location={ccLocation} />;
  }

  // Car detailing location (existing logic unchanged below)
  const location = getLocationBySlug(locationSlug);
  ...
  const faqItems = isTown
    ? [
        {
          question: `Do you offer car detailing for customers from ${location.name}?`,
          answer: `Yes — Car Ceramic Coating and detailing warrington0 serves customers from ${location.name} (${location.postcode}). Bring your vehicle to our unit at Fairclough Mill, Atherton\'s Quay, Warrington WA5 1AH for professional ceramic coating, machine polishing, car detailing, and more.`,
        },
        {
        ...
        },
        {
          ...
          answer: `We offer our full service range in ${location.name}, including car detailing, ceramic coating, machine polishing, interior detailing, and headlight restoration. All services are carried out to the same standard at our Warrington studio.`,
        },
        {
          question: `How much does car detailing cost in ${location.name}?`,
          answer: `Prices start from £80 for an exterior or interior detail, £200 for machine polishing, and £350 for ceramic coating. Final prices depend on vehicle size and condition. Contact us for a free, no-obligation quote specific to your vehicle.`,
        },
        {
    ...
    : [
        {
          question: `Do you serve customers from ${location.name}, Warrington?`,
          answer: `Yes — Car Ceramic Coating and detailing warrington0 serves customers from ${location.name} (${location.postcode}). Bring your vehicle to our unit at Fairclough Mill, Atherton\'s Quay, Warrington WA5 1AH — just a short drive away — for professional ceramic coating, machine polishing, car detailing, and more.`,
        },
        {
          question: `Which services do you offer for ${location.name} customers?`,
          answer: `We offer the full range including car detailing, ceramic coating, machine polishing, interior detailing, and headlight restoration. All services are available for ${location.name} customers at our Warrington unit.`,
        },
        {
          question: `How do I book a car detail in ${location.name}?`,
          answer: `Simply call us on ${PHONE_DISPLAY} or fill in our online quote form at /contact. Let us know your vehicle and what service you're interested in and we'll get back to you with a fixed-price quote. Then bring your vehicle to our unit at Fairclough Mill, Atherton\'s Quay, Warrington WA5 1AH.`,
        },
        {
          question: `Can you detail more than one car from ${location.name} in a single booking?`,
          answer: `Yes — we're happy to detail multiple vehicles. This is popular with households in ${location.name} where two or more cars need attention. Combined bookings also offer better value.`,
        },
      ];
    ...
    <>
      <JsonLd
        data={serviceSchema(
          ...
          `Professional car detailing in ${location.name}. Car Ceramic Coating and detailing warrington0 serves ${location.postcode} from our unit at Fairclough Mill, Atherton\'s Quay, Warrington WA5.`,
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="mt-6 max-w-3xl">
            <p className="section-subheading">
              {isWarringtonArea ? `Warrington ${location.postcode}` : `Car Detailing — ${location.postcode}`}
            </p>
            ...
            <p className="text-gray-300 text-lg leading-relaxed mb-8">
              Car Ceramic Coating and detailing warrington0 provides professional{' '}
              ...
              . Our dedicated unit in Warrington WA5 delivers showroom-quality results for
              customers from {location.name} and surrounding areas.
            </p>
            ...
            <div>
              ...
              <div className="space-y-4 text-gray-300 leading-relaxed">
                ...
                {isTown && (
                  <p>
                    Based at{' '}
                    <strong className="text-brand-white">Unit 1 Fairclough Mill, Atherton\'s Quay, Warrington WA5 1AH</strong>
                ...
                {isWarringtonArea && (
                  <p>
                    ...
                    of a truly professional detailing unit in Warrington WA5.
                  </p>
                )}
              </div>
              {isTown && (
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-14">
            ...
            <p className="text-gray-400 mt-4 max-w-2xl mx-auto">
              ...
              {isWarringtonArea ? `, Warrington` : ''} — all delivered at our unit at Fairclough Mill, Atherton\'s Quay, Warrington WA5.
            </p>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-heading font-bold text-brand-white mb-6">
                Why {location.name} Chooses Car Ceramic Coating and detailing warrington0
              </h2>
              <p className="text-gray-300 leading-relaxed mb-6">{location.whyWCD}</p>
              <div className="space-y-3">
                {[
                  'Professional unit-based service in Warrington, serving ' + location.name,
